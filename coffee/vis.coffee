...
Network = () ->
  ...
  charge = (node) -> -Math.pow(node.radius, 2) / 2
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        element.style("fill", "#F38630")
           # Here we need to make the element huge and colored if it's a match
  ...
  setupData = (data) ->
    ...
    circleRadius = d3.scale.sqrt().range([1, 4]).domain(countExtent)
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      ...
      # Here we define "popular" as in top 1/4 quantile
      cutoff = d3.quantile(playcounts, .25)
  ...
  showDetails = (d,i) ->
    ...
    content += '<p class="main"><a href="author.php?"' + d.id + '">view</a></span></p>'
...
$ ->
  ...
  d3.json "data/PInetwork_3383level_80.json", (json) ->
    myNetwork("#vis", json)
