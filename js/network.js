...
Network = function() {
  ...
  charge = function(node) {
    return -Math.pow(node.radius, 2) / 2;
  };
  ...
  network.updateSearch = function(searchTerm) {
    ...
    return node.each(function(d) {
      ...
      if (searchTerm.length > 0 && match >= 0) {
        ...
        }).style("stroke-width", 1.0).attr("r", d.radius);
      }
    });
  };
  ...
  setupData = function(data) {
    ...
    data.nodes.forEach(function(n) {
      ...
      return n.url = "author.php?uid=" + n.id;
    });
  ...
  setLayout = function(newLayout) {
    layout = newLayout;
    if (layout === "force") {
      return force.on("tick", forceTick).charge(-10).linkDistance(2);
...
$(function() {
  ...
  return d3.json("data/PInetwork_3383level_80.json", function(json) {
